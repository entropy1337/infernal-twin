...
sh /usr/local/etc/raddb/certs/bootstrap'''
	...
	def sslStrip(self, e):
		try:
			...
			hack_wireless_interface = str(self.ask(message = 'Enter the Wireless Interface name which are you testing from')).strip()
...
''')
		os.system('''rm /var/www/index.html''')
...
class authorwindows(wx.Frame):
	def __init__(self, parent, title):
		wx.Frame.__init__(self, parent, -1, title)
		html = wx.html.HtmlWindow(self)
		html.SetPage("<h4>Author: </h4>3nt0py<br>Email: 3ntr0py1337@gmail.com<br>LinkedIn: <a href='https://ae.linkedin.com/pub/mo-halilov/40/43a/5b3'> https://ae.linkedin.com/pub/mo-halilov/40/43a/5b3</a><br><h4>Contributors:</h4><br>@ndrix and zstyblik")
...
class WPA2_crack(wx.Frame):
    ...
    def __init__(self,parent,id):
        ...
        self.nDeviceList = wx.ComboBox(btnPanel, -1, value='Wireless Interface', size=(100, 30), pos=(2,100),choices = wless_commands.get_net_devices())
    ...
    def wpa_crack_key(self, SSID, channel):
		...
		if 'mon0' not in wless_commands.get_net_devices():
			...
			command = ("airodump-ng --essid '%s' --write './Modules/capture/%s_crack' %s -c %s &"% (SSID, SSID, self.nDeviceList.GetValue(), mychannel))
			print command 
		
		else:
			monitoring_interface = 'mon0'
			command = ("airodump-ng --essid '%s' --write './Modules/capture/%s_crack' %s -c %s &"% (SSID, SSID, monitoring_interface, mychannel))
