        ...
        if pkt.haslayer(Dot11Beacon):
            ...
            while temp:
                ...
                if temp and temp.ID == 0 and (pkt.addr3 not in ssids):
                    ...
                    # ~
                    tssid = str('[MAC Addr]-> ' + str(bssid) + ' : [ESSID]-> ' + repr(essid) + ' \t\t: [Encr]-> ' + str(
                    ...
                    if tssid not in new_ssid:
                        ...
                        ######## write to file #######
                        with open('./Modules/access_list.txt', 'a') as f:
                            f.write(tssid + '\n')

                        with open('./Modules/deauthSSID.txt', 'a') as g:
                            g.write(tempSSID2 + ',')
                        ######## write to file #######
            ...
            if pkt.type == 2 and pkt.subtype == 4:
                if pkt.addr1 + ' : ' + pkt.addr2 not in found_client_mac:
                    if pkt.addr1 in ssid_dictionary.keys():
                        ...
                        with open('./Modules/connection_list.txt', 'a') as f:
                            f.write(pkt.addr1 + ' : ' + pkt.addr2 + ' : ' + ssid_dictionary[pkt.addr1] + '\n')
                        with open('./Modules/deauth_ssid_client.txt', 'a') as g:
                            g.write(pkt.addr1 + '-' + pkt.addr2 + '-' + ssid_channel_dictionary[pkt.addr1] + '\n')
...
class SnifferGUI(wx.Frame):
    ...
    def onQuitApp(self, event):
        ...
        os.system(
            'echo ""> ./Modules/connection_list.txt;echo ""> ./Modules/access_list.txt;echo ""> ./Modules/probe_list.txt')
    ...
    def on_timer(self):
        ...
        os.system('iwconfig mon0 channel %d' % rChannel)
        os.system(
            'touch ./Modules/probe_list.txt; touch ./Modules/connection_list.txt; touch ./Modules/access_list.txt')
        probe_list = open('./Modules/probe_list.txt', 'r').readlines()

        connection_list = open('./Modules/connection_list.txt', 'r').read()
        ssid_list = str(open('./Modules/access_list.txt', 'r').read())
        probe_info = ''
        global client_macs
