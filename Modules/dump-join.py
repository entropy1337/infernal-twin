...
def parse_file(file, file_name):
    ...
    try:
        header = cleanup.index(
            'BSSID, First time seen, Last time seen, channel, Speed, Privacy, Cipher, Authentication, Power, # beacons, # IV, LAN IP, ID-length, ESSID, Key')
        stationStart = cleanup.index(
    ...
    del cleanup[
        stationStart:]  # removed all of the client info leaving only the info on available target AP's in ardump maby i should create a new list for APs?
...
if __name__ == "__main__":
    ...
    parser.add_option("-i", "--file", dest="filename", nargs=2,
                      help="Input files to read data from requires at least two arguments")
