...
class SnifferGUI(wx.Frame):
	...
	def InitUI(self):
		...
		self.nDeviceList = wx.ComboBox(panel, -1, value='Select Net Interface', size=(200, 30), pos=(10, 90), choices = wless_commands.get_net_devices())
	...
	def generateImageAction(self, e):
		...
		mcommand = "airgraph-ng -i Modules/Logs/"+str(dumpfilename)+"/"+str(dumpfilename)+"*.csv -o Modules/Logs/"+str(dumpfilename)+"/"+str(dumpfilename)+".png -g CAPR"
	...
	def executeCommand(self,e):
		...
		try:
			...
			if attackInterface == 'mon0':
				...
				os.system("airodump-ng -w ./Modules/Logs/"+str(dumpfilename)+"/"+str(dumpfilename)+" "+attackInterface + "&")
