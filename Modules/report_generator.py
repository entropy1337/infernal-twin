...
class Example(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.cb = wx.ComboBox(panel, -1, "Choose a category", pos=(10, 140), choices=section_category,
        ...
        self.cb3 = wx.ComboBox(panel, -1, "Choose risk level", pos=(250, 100), choices=section_risk,
                               style=wx.CB_READONLY)

        self.findings = wx.MultiLine = wx.TextCtrl(panel, -1, "Enter the findings details", pos=(10, 200),
                                                   size=(510, 220), style=wx.TE_MULTILINE)
        # self.targetname.SetBackgroundColour('#ededed')

        self.notes = wx.MultiLine = wx.TextCtrl(panel, -1, "Notes on the findings", pos=(10, 450), size=(510, 100),
        ...
        cb = wx.ComboBox(panel, pos=(200, 600), choices=project_name, size=(280, 30), value='Select a Project',
                         style=wx.CB_READONLY)
    ...
    def create_report(self, finding_name, phase, risk_level, risk_category, Findings_detail, Notes):
        ...
        REPORT_DETAILS = 'INSERT INTO Reports (finding_name, phase, risk_level, risk_category, Findings_detail, Notes, Project_fk_Id) VALUES ("%s","%s","%s","%s","%s","%s","%d")' % (
        finding_name, phase, risk_level, risk_category, Findings_detail, Notes, int(pID))
    ...
    def create_project(self, e):
        ...
        if self.rtitle != " " or self.rfindings != " ":
            ...
            self.create_report(str(self.rtitle), str(self.rphase), str(self.rcategory), str(self.rrisklevel),
                               str(self.rfindings), str(self.rnotes))
	      ...
	      <tbody>
            ...
            self.rtitle, self.rphase, self.rfindings, self.rnotes, self.rrisklevel, self.rcategory)
...
#category {  
  ...
  <body><h1>Report </h1><br><br><br><br><br>
    <p>Project: %s<br>
        ...
        os.system(
            'firefox ' + str(self.fname).replace(' ', '_') + "/" + str(self.fname).replace(' ', '_') + ".html" + " &")
